...
def fix_future_accruals(tm_id):
    """Delete v4_import accrual rows dated after today so the employee
    stops showing future PTO as already accrued."""
    import db as _db
    ...
    conn.commit()
    conn.close()
    flash(f"Removed {deleted} future accrual rows for {tm_id}. Balance will update shortly.", "success")
    return redirect(url_for("pto.pto_page"))
